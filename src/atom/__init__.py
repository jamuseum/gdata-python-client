...
class ExtensionContainer(object):
  ...
  def _AddMembersToElementTree(self, tree):
    ...
    for attribute, value in six.iteritems(self.extension_attributes):
      if value:
        if isinstance(value, six.text_type) or MEMBER_STRING_ENCODING is six.text_type:
    ...
    if self.text:
      if isinstance(self.text, six.text_type) or MEMBER_STRING_ENCODING is six.text_type:
        tree.text = self.text
      else:
...
def _ExtensionElementFromElementTree(element_tree):
  ...
  for key, value in six.iteritems(element_tree.attrib):
