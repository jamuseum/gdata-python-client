  ...
  if scopes_included_in_next:
    for pair in parsed[4].split('&'):
      if pair.startswith('%s=' % SCOPE_URL_PARAM_NAME):
        scopes = six.moves.urllib.parse.unquote_plus(pair.split('=')[1])
...
def GenerateAuthSubRequestUrl(next, scopes, hd='default', secure=False,
  ...
  if include_scopes_in_next:
    if next.find('?') > -1:
      next += '&%s' % six.moves.urllib.parse.urlencode({SCOPE_URL_PARAM_NAME:scope})
    else:
      next += '?%s' % six.moves.urllib.parse.urlencode({SCOPE_URL_PARAM_NAME:scope})
...
class Query(dict):
  ...
  def ToUri(self):
    ...
    category_string = '/'.join(
        [six.moves.urllib.parse.quote_plus(c) for c in self.categories])
