  ...
  def _ServiceUrl(self, service_type, is_existed, group_id, member_id, owner_email,
    ...
    if service_type == 'group':
      if group_id != '' and is_existed:
        ...
        if direct_only:
          return GROUP_MEMBER_DIRECT_URL % (domain, six.moves.urllib.parse.quote_plus(member_id),
                                            self._Bool2Str(direct_only))
        else:
          return GROUP_MEMBER_URL % (domain, six.moves.urllib.parse.quote_plus(member_id))
      else:
        return BASE_URL % (domain)

    if service_type == 'member':
      if member_id != '' and is_existed:
        return MEMBER_ID_URL % (domain, group_id, six.moves.urllib.parse.quote_plus(member_id))
    ...
    if service_type == 'owner':
      if owner_email != '' and is_existed:
        return OWNER_ID_URL % (domain, group_id, six.moves.urllib.parse.quote_plus(owner_email))
      elif suspended_users:
